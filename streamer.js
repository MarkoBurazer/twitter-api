...
function startStream() {
   ...
   **/
  client.stream('statuses/filter', {track: search_queries.toString() },  function(stream) {
    stream.on('data', function(tweet) {
      ...
      console.log(log);
      fs.appendFile('tweets/'+file_name+'.json', string + ',\n', function(err) {
        if(err) console.log(err);
      });
      fs.appendFile('tweets/logs.txt', log + ',\n', function(err) {
        if(err) console.log(err);
      });
    });
    ...
    stream.on('error', function(error) {
      console.log(error);
      var log = moment().format(time_format);
      fs.appendFile('logs-offline.txt', log + ',\n', function(err) {
        if(err) console.log(err);
      });
    });
